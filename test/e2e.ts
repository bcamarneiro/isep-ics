...
async function waitForService(maxRetries: number = MAX_RETRIES, delay: number = RETRY_DELAY): Promise<boolean> {
...
function printEventSummary(events: any[], title: string, maxEvents: number = 5) {
  ...
  for (let i = 0; i < Math.min(events.length, maxEvents); i++) {
    ...
    const summary = event.summary ? (event.summary.length > 50 ? event.summary.slice(0, 50) + '...' : event.summary) : 'No title';
